...
- Command history now preserves chronological order by default
  - Introduced `FZF_ACTION_COMMAND_HISTORY_FZF_OPTS` environment variable for customizable fzf options
  - Defaults to `--no-sort` to maintain history order (newest first)
  - Users can enable sorting by setting `FZF_ACTION_COMMAND_HISTORY_FZF_OPTS=""`
  - Extended `fzf-action-core` to accept optional `extra_fzf_opts` parameter for future extensibility

## [1.1.3] - 2025-11-13

### Fixed

- Merged PR #23: Improved asterisk extraction in command-history widget
  - Fixed pattern matching to correctly extract commands from history entries with asterisk markers
  - Enhanced robustness of command parsing logic

## [1.1.2] - 2025-11-12

### Fixed

- Merged PR #21: Fixed command extraction in command-history widget to handle asterisk-marked entries
